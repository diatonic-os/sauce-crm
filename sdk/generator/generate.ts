...
import { loadTasksSchema, writeTasksModule } from "./introspect-tasks";
...
function main(): void {
  ...
  if (!root) {
    console.error(
      "[sdk:gen] Obsidian docs not found. Set SAUCE_OBSIDIAN_DOCS or vendor reference/obsidian-developer-docs.",
    );
    process.exit(1);
  }
  ...
  // CON-OBS-INTEG-001 SH-F: deterministic obsidian-tasks codegen from the
  // committed schema (the live introspector writes the schema separately).
  writeTasksModule(loadTasksSchema());
