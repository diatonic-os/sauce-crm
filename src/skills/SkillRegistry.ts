...
export interface SkillSettings { enabled: boolean; autonomy: AutonomyLevel; providerOverride?: string; }
...
export class SkillRegistry {
  ...
  constructor() {
    for (const s of [
      new ResearchOrgSkill(), new ResearchPersonSkill(), new DraftTouchSkill(),
      new SummarizeThreadSkill(), new CaptureCallSkill(), new InferEdgesSkill(),
      new GeocodeSkill(), new TranscribeSkill(), new RouteIntroductionSkill(),
      new ImportContactsSkill(), new ExportGraphSkill(), new ScheduleTouchSkill(),
      new SummarizeWeekSkill(), new MergeDuplicatesSkill(), new VerifyEmailSkill(),
  ...
  enabled(): Skill[] { return this.list().filter((s) => this.settings.get(s.id)?.enabled !== false); }
  ...
  getSettings(id: string): SkillSettings { return this.settings.get(id) ?? { enabled: true, autonomy: 'propose' }; }
}
