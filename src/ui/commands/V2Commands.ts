...
export interface V2CommandDescriptor {
  ...
  category: 'capture' | 'view' | 'skill' | 'vault' | 'security' | 'sync' | 'import-export' | 'inference';
}
...
export function registerV2Commands(opts: {
  addCommand: (cmd: { id: string; name: string; hotkeys?: { modifiers: string[]; key: string }[]; callback: () => void | Promise<void> }) => void;
