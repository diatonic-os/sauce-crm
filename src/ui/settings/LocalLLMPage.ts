...
export class LocalLLMPage extends SettingsPage {
  ...
  render(containerEl: HTMLElement): void {
    ...
    this.section(containerEl, "Ollama", [
      {
        label: "Endpoint URL",
        key: "copilot.ollama.endpoint",
        placeholder: "http://localhost:11434",
        secret: false,
      },
      {
        label: "API key (optional, for reverse-proxied Ollama)",
        key: "copilot:ollama:api-key",
        placeholder: "leave blank if Ollama has no auth",
        secret: true,
      },
    ]);
    this.modelPicker(
      containerEl,
      "ollama",
      "copilot.ollama.endpoint",
      "copilot.ollama.defaultModel",
    );

    this.section(containerEl, "LM Studio", [
      {
        label: "Endpoint URL (OpenAI-compatible base)",
        key: "copilot.lmstudio.endpoint",
        placeholder: "http://localhost:1234/v1",
        secret: false,
      },
      {
        label: "API key (optional)",
        key: "copilot:lmstudio:api-key",
        placeholder: "leave blank for default LM Studio setup",
        secret: true,
      },
    ]);
    this.modelPicker(
      containerEl,
      "lmstudio",
      "copilot.lmstudio.endpoint",
      "copilot.lmstudio.defaultModel",
    );

    const toggleWrap = containerEl.appendChild(
      el("div", { class: "sauce-settings-row" }),
    );
    toggleWrap.appendChild(
      el("label", {}, "LM Studio tool-use (OpenAI function-calling)"),
    );
    const tg = toggleWrap.appendChild(el("input")) as HTMLInputElement;
    tg.setAttribute("type", "checkbox");
    tg.checked = this.host.getConfig(
      "copilot.lmstudio.toolUse",
      false,
    ) as boolean;
    ...
    const pingOllama = actions.appendChild(
      el("button", {}, "Ping Ollama"),
    ) as HTMLButtonElement;
    const pingLM = actions.appendChild(
      el("button", {}, "Ping LM Studio"),
    ) as HTMLButtonElement;
    const statusEl = actions.appendChild(
      el("span", { class: "sauce-settings-status" }),
    );
    pingOllama.addEventListener("click", async () => {
      const fn = this.host.getConfig<
        | (() => Promise<{ ok: boolean; latencyMs: number; error?: string }>)
        | null
      >("copilot.ollama.pingFn", null);
      statusEl.textContent = fn
        ? await this.formatPing("Ollama", fn)
        : "Ping handler not wired";
    });
    pingLM.addEventListener("click", async () => {
      const fn = this.host.getConfig<
        | (() => Promise<{ ok: boolean; latencyMs: number; error?: string }>)
        | null
      >("copilot.lmstudio.pingFn", null);
      statusEl.textContent = fn
        ? await this.formatPing("LM Studio", fn)
        : "Ping handler not wired";
    });
  }
  ...
  private modelPicker(
    ...
    if (!plugin) {
      parent.appendChild(
        el(
          "div",
          { class: "sauce-settings-hint" },
          "Plugin handle not wired — pickers disabled. (settings-host must expose plugin.handle for live model lookup)",
        ),
      );
      return;
    }
    ...
    new ProviderPicker({
      ...
      onChange: ({ model }) => {
        void this.host.setConfig(defaultKey, model);
      },
    }).render();
  }

  private async formatPing(
    name: string,
    ...
    return r.ok
      ...
      : `${name}: ${r.error ?? "fail"} (${r.latencyMs}ms)`;
  }
