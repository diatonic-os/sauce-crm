...
import { PROVIDER_HELP, helpForEmail, helpById, type ProviderHelpEntry } from '../../../integrations/smtpimap/HelpLinks';
...
export interface SmtpImapPageHost extends SettingsHost {
  ...
  testConnection?(accountId: string): Promise<{ ok: boolean; message: string; latencyMs?: number }>;
}
...
export class SmtpImapPage extends SettingsPage {
  ...
  constructor(private readonly host: SmtpImapPageHost) { super(); }
  ...
  render(containerEl: HTMLElement): void {
    ...
    for (const field of [
      ...
      inp.addEventListener('change', () => {
        ...
        if (field.key === 'account.username') this.autoDetect(inp.value, inputs);
      });
    ...
    helpContainer.appendChild(el('p', { class: 'sauce-settings-hint' },
    ...
    for (const entry of PROVIDER_HELP) {
      ...
      if (entry.appPasswordUrl) {
        const btn = header.appendChild(el('button', { class: 'mod-cta sauce-help-link' }, 'Open app password page')) as HTMLButtonElement;
        const url = entry.appPasswordUrl;
        btn.addEventListener('click', () => {
          if (this.host.openExternal) this.host.openExternal(url);
          else if (typeof window !== 'undefined' && (window as { open?: (u: string) => void }).open) (window as { open: (u: string) => void }).open(url);
        });
      }
      if (entry.oauthSetupUrl) {
        const ob = header.appendChild(el('button', { class: 'sauce-help-link-secondary' }, 'OAuth setup')) as HTMLButtonElement;
        const ourl = entry.oauthSetupUrl;
        ob.addEventListener('click', () => {
          if (this.host.openExternal) this.host.openExternal(ourl);
          else if (typeof window !== 'undefined' && (window as { open?: (u: string) => void }).open) (window as { open: (u: string) => void }).open(ourl);
        });
      }
    ...
    const testBtn = actions.appendChild(el('button', { class: 'mod-cta' }, 'Test connection')) as HTMLButtonElement;
    const status = actions.appendChild(el('span', { class: 'sauce-settings-status' }));
    testBtn.addEventListener('click', async () => {
      const accountId = inputs['account.id'].value || 'default';
      if (!this.host.testConnection) { status.textContent = 'Test handler not wired'; return; }
    ...
    // Security notice
    containerEl.appendChild(el('div', { class: 'sauce-security-notice' },
  ...
  private autoDetect(email: string, inputs: Record<string, HTMLInputElement>): void {
