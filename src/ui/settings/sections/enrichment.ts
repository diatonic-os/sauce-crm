...
export function renderEnrichment(containerEl: HTMLElement, plugin: SauceGraphPlugin): void {
  ...
  addToggleRow(containerEl, {
    ...
    value: e.enabled,
    onChange: async (v) => { e.enabled = v; await save(); },
  });
  ...
  addToggleRow(containerEl, {
    ...
    value: e.autostart,
    onChange: async (v) => { e.autostart = v; await save(); },
  });
  ...
  addToggleRow(containerEl, {
    ...
    value: e.classify,
    onChange: async (v) => { e.classify = v; await save(); },
  });
  addToggleRow(containerEl, {
    name: "Stage: tag",
    desc: "Extract topics / #hashtags into the tags list.",
    value: e.tag,
    onChange: async (v) => { e.tag = v; await save(); },
  });
  addToggleRow(containerEl, {
    name: "Stage: graph",
    desc: "Derive edges from [[wikilinks]] / mentions in the body.",
    value: e.graph,
    onChange: async (v) => { e.graph = v; await save(); },
  });
}
