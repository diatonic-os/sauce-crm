...
const THIRD_PARTY_WIRED: CredentialProviderId[] = [
  "google_workspace",
  "microsoft_365",
  "notion",
  "twilio",
...
const THIRD_PARTY_SOON: { name: string; blurb: string; phase: string }[] = [
  {
    name: "Airtable",
    blurb: "Two-way sync of bases ↔ vault entities.",
    phase: "P15",
  },
  {
    name: "Wix Studio",
    blurb: "Publish and sync CMS collections into your graph.",
    phase: "P15",
  },
  {
    name: "Supabase",
    blurb: "Mirror entities to Postgres for realtime apps.",
    phase: "P15",
  },
];
...
function providerBlurb(id: CredentialProviderId): string {
  switch (id) {
    case "google_workspace":
      return "Calendar, Mail, Contacts, Drive. PKCE flow with your own Google OAuth client.";
    case "microsoft_365":
      return "Calendar, Mail, Contacts, OneDrive. PKCE flow with your own Microsoft Entra app.";
    case "notion":
      return "Workspace pages + databases. Uses an internal integration token.";
    case "twilio":
...
async function summarizeCreds(
  plugin: SauceGraphPlugin,
  id: CredentialProviderId,
): Promise<{ text: string; btn: string; ok: boolean }> {
  const creds = plugin.credentials;
  if (!creds)
    return {
      text: "KeyVault not initialized — credentials unavailable.",
      btn: "Open",
      ok: false,
    };
  if (plugin.keyVault?.isLocked())
    return {
      text: "Vault locked — click to unlock and configure.",
      btn: "Unlock & configure",
      ok: false,
    };
  ...
  if (m.kind === "oauth") {
    ...
    return cid
      ? {
          text: `Client ID stored (${cid.slice(0, 8)}…)`,
          btn: "Edit",
          ok: true,
        }
      : {
          text: "Not configured — paste OAuth client ID.",
          btn: "Configure",
          ok: false,
        };
  } else if (m.keyFields) {
    for (const f of m.keyFields) {
      if (await creds.getKey(id, f.id))
        return { text: "Credentials saved.", btn: "Edit", ok: true };
    }
...
function cardShell(
  ...
  badgeText?: string,
  badgeMod = "",
): { card: HTMLElement; foot: HTMLElement } {
  ...
  if (badgeText)
    head.createEl("span", {
      cls: `sauce-badge${badgeMod ? " " + badgeMod : ""}`,
      text: badgeText,
    });
...
async function buildProviderCard(
  ...
  const { card, foot } = cardShell(
    grid,
    m.label,
    m.kind === "oauth" ? "OAuth" : "API key",
    "sauce-badge--muted",
  );
  // Insert blurb + status above the footer (which cardShell appended last).
  const blurb = createDiv({
    cls: "sauce-card-meta",
    text: providerBlurb(credId),
  });
  const status = createDiv({
    cls: "sauce-card-status",
    text: "Checking credentials…",
  });
  card.insertBefore(blurb, foot);
  card.insertBefore(status, foot);

  const cfgBtn = foot.createEl("button", {
  ...
  const integ = (
    plugin.integrations as { byId?: (id: string) => unknown } | null
  )?.byId?.(credId) as {
    connect?: () => Promise<unknown>;
    disconnect?: () => Promise<void>;
    state?: () => Promise<{ connected: boolean; expiresAt?: number }>;
  } | null;
  if (!integ) return;
  let conn = { connected: false } as { connected: boolean; expiresAt?: number };
  try {
    conn = (await integ.state?.()) ?? conn;
  } catch {
    /* ignore */
  }
  status.setText(
    conn.connected
      ? conn.expiresAt
        ? `Connected · expires in ${Math.max(0, Math.floor((conn.expiresAt - Date.now()) / 60_000))}m`
        : "Connected"
      : s.text,
  );
  status.toggleClass("is-ok", conn.connected || s.ok);
  const connBtn = foot.createEl("button", {
  ...
  connBtn.onclick = async () => {
    try {
      ...
      plugin.logger?.event?.("integrations.connect_toggle", {
        provider: credId,
...
function buildComingSoonCard(
  grid: HTMLElement,
  cs: { name: string; blurb: string; phase: string },
): void {
...
function buildCommunityCard(
  ...
  if (st.enabled) {
    foot.createEl("button", {
      cls: "sauce-btn sauce-btn--secondary",
      text: "Open settings",
    }).onclick = () => openCommunityPluginsPage(plugin.app as never);
    ...
    b.onclick = async () => {
      try {
        await (
          app.plugins?.enablePluginAndSave ?? app.plugins?.enablePlugin
        )?.call(app.plugins, st.spec.id);
      } finally {
        rerender();
      }
    };
...
export function renderIntegrations(
  ...
  plugin.logger?.debug?.("settings.section_render", {
    section: "integrations",
  });
  const rerender = () => {
    containerEl.empty();
    renderIntegrations(containerEl, plugin);
  };
  ...
  for (const id of AI_PROVIDERS)
    void buildProviderCard(aiGrid, plugin, id, rerender);

  // ── 2. 3rd-party tools ────────────────────────────────────────────
  sectionTitle(
    containerEl,
    "3rd-party tools",
    "Productivity and data services. Connected accounts sync into your vault graph.",
  );
  const toolGrid = containerEl.createDiv({ cls: "sauce-card-grid" });
  for (const id of THIRD_PARTY_WIRED)
    void buildProviderCard(toolGrid, plugin, id, rerender);
  ...
  sectionTitle(
    containerEl,
    "Community plugins",
    "Obsidian plugins Sauce CRM works best with. Install opens the official community-plugin page — nothing is installed silently.",
  );
  const pluginGrid = containerEl.createDiv({ cls: "sauce-card-grid" });
  const statuses = detectCommunityPlugins(plugin.app as never);
  for (const st of statuses)
    buildCommunityCard(pluginGrid, plugin, st, rerender);
  // Core plugins entry point — toggled in Obsidian's own settings tab.
  const { foot } = cardShell(
    pluginGrid,
    "Core plugins",
    "Built-in",
    "sauce-badge--muted",
