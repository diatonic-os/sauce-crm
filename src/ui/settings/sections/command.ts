...
class SlashCommandModal extends Modal {
  ...
  onOpen(): void {
    ...
    new Setting(contentEl)
      .setName("Prompt")
      .setDesc(
        "Use {} where the selected text / active note should be inserted.",
      )
      .addTextArea((t) => {
...
export function renderCommand(
  ...
  const actions = containerEl.createDiv({
    cls: "sauce-hero-row sauce-row-end",
  });
  ...
  addBtn.onclick = () =>
    new SlashCommandModal(
      plugin.app,
      { id: "", name: "", prompt: "{}", inMenu: true, slashCmd: true },
      async (c) => {
        c.id = slug(c.name);
        cmds.push(c);
        await save();
  ...
  cmds.forEach((cmd, i) => {
    ...
    const iconBtn = (icon: string, label: string, fn: () => void): void => {
      const b = actCell.createEl("button", {
        cls: "sauce-icon-btn",
