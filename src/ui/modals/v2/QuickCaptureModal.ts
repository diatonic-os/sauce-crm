...
export class QuickCaptureModal extends Modal {
  ...
  onOpen(): void {
    ...
    c.createEl("p", { cls: "sauce-quick-hint", text: "Type CDEL directives or natural language. Examples: `@person Jane Smith` · `@touch Jane today coffee` · `@org Acme industry:saas`. Cmd+Enter to dispatch." });

    this.input = c.createEl("textarea", { cls: "sauce-quick-input" });
    this.input.rows = 6;
    this.input.placeholder = "@person Jane Smith\nemail: jane@example.com\nroles: [advisor]";
    ...
    this.input.addEventListener("keydown", (ev) => {
      if ((ev.metaKey || ev.ctrlKey) && ev.key === "Enter") { ev.preventDefault(); void this.dispatch(); }
    });

    const btns = c.createDiv({ cls: "sauce-buttons" });
    btns.createEl("button", { text: "Dispatch (Cmd+Enter)", cls: "sauce-button" }).onclick = () => void this.dispatch();
    btns.createEl("button", { text: "Cancel", cls: "sauce-button sauce-button-secondary" }).onclick = () => this.close();

    setTimeout(() => this.input.focus(), 50);
  }

  private currentDispatches(): { dispatches: CdelDispatch[]; unhandled: string[] } {
    const src = this.input.value.trim();
    if (!src) return { dispatches: [], unhandled: [] };
    try { return this.interpreter.interpret(src); }
    catch (e: any) { return { dispatches: [], unhandled: [`parse error: ${e?.message ?? e}`] }; }
  }

  private refresh(): void {
    const { dispatches, unhandled } = this.currentDispatches();
    const lines: string[] = [];
    for (const d of dispatches) lines.push(`→ ${d.skillId}  ${JSON.stringify(d.args).slice(0, 120)}`);
  ...
  private async dispatch(): Promise<void> {
    ...
    const { dispatches, unhandled } = this.currentDispatches();
    if (dispatches.length === 0) { new Notice("nothing to dispatch"); return; }
    let ok = 0, failed = 0;
    ...
    new Notice(`Quick Capture: ${ok} ok, ${failed} failed, ${unhandled.length} unhandled`);
    this.close();
  }
