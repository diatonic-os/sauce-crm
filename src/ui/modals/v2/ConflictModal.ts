...
export class ConflictModal extends Modal {
  ...
  onOpen(): void {
    ...
    c.createEl("p", { cls: "sauce-conflict-hint", text: "Pick the value to keep per field. 'Both' concatenates lists / preserves history." });
    ...
    for (const f of this.fields) {
      ...
      for (const opt of ["local", "remote", "both", "skip"]) sel.createEl("option", { text: opt, value: opt });
      sel.value = "local";
      this.decisions[f.name] = "local";
      sel.onchange = () => { this.decisions[f.name] = sel.value as any; };
    }

    const btns = c.createDiv({ cls: "sauce-buttons" });
    btns.createEl("button", { text: "Resolve", cls: "sauce-button" }).onclick = () => this.resolve();
    btns.createEl("button", { text: "Cancel", cls: "sauce-button sauce-button-secondary" }).onclick = () => { this.onResolve(null); this.close(); };
  }
    ...
    for (const f of this.fields) {
      ...
      if (choice === "local") resolved[f.name] = f.local;
        ...
        if (Array.isArray(f.local) && Array.isArray(f.remote)) {
          resolved[f.name] = [...new Set([...(f.local as any[]), ...(f.remote as any[])])];
        } else {
