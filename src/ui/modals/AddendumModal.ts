...
export class AddendumModal extends Modal {
  ...
  constructor(public app: App, public plugin: SauceGraphPlugin, target: TFile | null = null) {
    super(app);
  ...
  onOpen(): void {
    ...
    new Setting(contentEl).setName("Addends (target)").addButton((b) => b
      .setButtonText(this.fm.addends ?? "Pick target")
      .onClick(() => {
    ...
    new Setting(contentEl).setName("Kind").addDropdown((d) => {
      for (const e of this.plugin.enums().kind_addendum ?? []) d.addOption(e, e);
      d.setValue(this.fm.kind); d.onChange((v) => (this.fm.kind = v));
    });

    new Setting(contentEl).setName("Date").addText((t) => t.setValue(this.fm.date).onChange((v) => (this.fm.date = v)));

    new Setting(contentEl).setName("Body").addTextArea((t) => t.onChange((v) => (this.body = v)));

    const btns = contentEl.createDiv({ cls: "sauce-buttons" });
    btns.createEl("button", { text: "Save (immutable)", cls: "sauce-button" }).onclick = () => this.save();
    btns.createEl("button", { text: "Cancel", cls: "sauce-button sauce-button-secondary" }).onclick = () => this.close();
  }

  async save(): Promise<void> {
    if (!this.fm.addends) { new Notice("Target is required"); return; }
    ...
    await this.plugin.entityService.createEntity(this.plugin.settings.paths.addenda, slug, fm, this.body);
