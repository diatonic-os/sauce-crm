...
import { todayIso, maxDate, parseIsoSafe, touchFolderForDate } from "../../util/DateUtil";
...
export class TouchModal extends Modal {
  ...
  constructor(public app: App, public plugin: SauceGraphPlugin, prefill?: Record<string, any>) {
    super(app);
  ...
  onOpen(): void {
    ...
    new Setting(contentEl).setName("Contact").addButton((b) => b
      .setButtonText(this.fm.contact ?? "Pick person")
      .onClick(() => {
        new WikilinkSuggest(this.app, [this.plugin.settings.paths.people], (_f, raw) => {
          this.fm.contact = wrapWikilink(raw);
          this.fm.attendees = uniq([this.fm.contact, ...(this.fm.attendees ?? [])]);
          b.setButtonText(this.fm.contact);
        }, false).open();
      }));

    new Setting(contentEl).setName("Date (YYYY-MM-DD)").addText((t) => t
      .setValue(this.fm.date).onChange((v) => (this.fm.date = v)));

    new Setting(contentEl).setName("Channel").addDropdown((d) => {
      for (const e of this.plugin.enums().channel ?? []) d.addOption(e, e);
      d.setValue("in-person"); d.onChange((v) => (this.fm.channel = v));
    });

    new Setting(contentEl).setName("Playbook").addDropdown((d) => {
      for (const e of (this.plugin.enums().playbook ?? ["ff-1","ff-2","ff-3","ff-4","ment-1","ment-2","ment-3","ment-4",""])) d.addOption(e, e || "(none)");
      d.setValue(""); d.onChange((v) => (this.fm.playbook_used = v));
    });

    new Setting(contentEl).setName("Outcome tags (comma-separated)").addText((t) => t
      .onChange((v) => (this.fm.outcome_tags = v.split(",").map((s) => s.trim()).filter(Boolean))));

    new Setting(contentEl).setName("Additional attendees").addButton((b) => b
      .setButtonText(`+ Add (${(this.fm.attendees ?? []).length})`)
      .onClick(() => {
        new WikilinkSuggest(this.app, [this.plugin.settings.paths.people], (_f, raw) => {
          this.fm.attendees = uniq([...(this.fm.attendees ?? []), wrapWikilink(raw)]);
          b.setButtonText(`+ Add (${this.fm.attendees.length})`);
        }).open();
      }));

    new Setting(contentEl).setName("Follow-ups (one per line)").addTextArea((t) => t
      .onChange((v) => (this.follows = v.split("\n").map((s) => s.trim()).filter(Boolean))));
  ...
  async save(): Promise<void> {
    if (!this.fm.contact) { new Notice("Contact is required"); return; }
    const d = parseIsoSafe(this.fm.date);
    if (!d) { new Notice("Invalid date — use YYYY-MM-DD"); return; }
    ...
    const slug = slugify(`${this.fm.date}-${contactName}-${this.fm.channel ?? "touch"}`);
    const fm = TemplateService.touchFrontmatter({ ...this.fm });
    const body = this.follows.length ? "\n\n## Follow-ups\n" + this.follows.map((f) => `- [ ] ${f}`).join("\n") : "";
    const file = await this.plugin.entityService.createEntity(folder, slug, fm, body);

    // bump person's last_touch
    const personFile = this.app.metadataCache.getFirstLinkpathDest(contactName, file.path);
