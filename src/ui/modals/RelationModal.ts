...
const EDGE_TYPES = ["knows","worked_with","intro_candidates","family_of","intro_via","parent"];
...
export class RelationModal extends Modal {
  ...
  constructor(public app: App, public plugin: SauceGraphPlugin, srcFile: TFile | null = null) {
    super(app);
  ...
  onOpen(): void {
    ...
    new Setting(contentEl).setName("Source").addButton((b) => b
      .setButtonText(this.srcFile?.basename ?? "Pick source")
      .onClick(() => new WikilinkSuggest(this.app, [], (f) => {
        if (f) { this.srcFile = f; b.setButtonText(f.basename); }
      }, false).open()));

    new Setting(contentEl).setName("Edge type").addDropdown((d) => {
      for (const e of EDGE_TYPES) d.addOption(e, e);
      d.setValue(this.edge); d.onChange((v) => (this.edge = v));
    });

    new Setting(contentEl).setName("Target").addButton((b) => b
      .setButtonText("Pick target")
      .onClick(() => new WikilinkSuggest(this.app, [], (_f, raw) => {
        this.target = raw; b.setButtonText(raw);
      }).open()));
  ...
  async save(): Promise<void> {
    if (!this.srcFile || !this.target) { new Notice("Source and target are required"); return; }
    const link = wrapWikilink(this.target);
    await this.plugin.entityService.updateFrontmatter(this.srcFile, (fm) => {
      const cur = Array.isArray(fm[this.edge]) ? fm[this.edge] : fm[this.edge] ? [fm[this.edge]] : [];
