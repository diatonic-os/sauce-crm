...
export class RegisterSubVaultModal extends Modal {
  ...
  override onOpen(): void {
    ...
    new Setting(contentEl)
      ...
      .addText((t) =>
        t.onChange(
          (v) => (this.vault_id = slugify(v).toLowerCase().replace(/\s/g, "-")),
        ),
      );
    new Setting(contentEl)
      .setName("Path (relative to ParentVault)")
      .addText((t) => t.setValue("./").onChange((v) => (this.path = v)));
    new Setting(contentEl).setName("Role").addDropdown((d) =>
      d
        ...
        .setValue("secondary")
        .onChange((v) => (this.role = v)),
    );

    const btns = contentEl.createDiv({ cls: "sauce-buttons" });
    btns.createEl("button", { text: "Register", cls: "sauce-button" }).onclick =
      () => this.save();
  ...
  async save(): Promise<void> {
    if (!this.vault_id) {
      new Notice("vault_id required");
      return;
    }
    const f = await this.plugin.registry.registerSubVault({
