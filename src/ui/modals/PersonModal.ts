...
export class PersonModal extends Modal {
  ...
  onOpen(): void {
    ...
    contentEl.createEl("h2", { text: this.editingFile ? `Edit Person — ${this.name}` : "New Person" });

    new Setting(contentEl).setName("Name").addText((t) => t
      .setPlaceholder("Full name")
      .setValue(this.name)
      .onChange((v) => (this.name = v)));

    new Setting(contentEl).setName("Primary type").addDropdown((d) => {
      for (const e of this.plugin.enums().primary_type_person ?? []) d.addOption(e, e);
      d.setValue(this.fm.primary_type ?? "");
      d.onChange((v) => (this.fm.primary_type = v));
    });

    new Setting(contentEl).setName("Roles (comma-separated)").addText((t) => t
      .setValue((this.fm.roles ?? []).join(", "))
      .onChange((v) => (this.fm.roles = v.split(",").map((s) => s.trim()).filter(Boolean))));

    new Setting(contentEl).setName("Closeness 1–5").addSlider((s) => s
      .setLimits(1, 5, 1).setValue(Number(this.fm.closeness ?? 3)).setDynamicTooltip()
    ...
    new Setting(contentEl).setName("Company").addButton((b) => b
      .setButtonText(this.fm.company ?? "Pick org")
      .onClick(() => {
    ...
    for (const edge of ["knows", "worked_with", "intro_candidates"]) {
      new Setting(contentEl).setName(edge).addButton((b) => b
        .setButtonText(`+ Add (${(this.fm[edge] ?? []).length})`)
        .onClick(() => {
          new WikilinkSuggest(this.app, [this.plugin.settings.paths.people], (_f, raw) => {
            this.fm[edge] = uniq([...(this.fm[edge] ?? []), wrapWikilink(raw)]);
            b.setButtonText(`+ Add (${this.fm[edge].length})`);
          }).open();
        }));
    }

    new Setting(contentEl).setName("intro_opt_in").addToggle((t) => t
  ...
  async save(): Promise<void> {
    ...
    if (!validation.passed) {
      new Notice("Contract violation:\n" + validation.violations.map((v) => `• ${v.invariant}`).join("\n"));
      return;
    }
    if (this.editingFile && this.editingFile.basename !== name) {
      await this.app.fileManager.renameFile(this.editingFile, `${this.plugin.settings.paths.people}/${name}.md`);
    }
    if (this.editingFile) {
      await this.plugin.entityService.updateFrontmatter(this.editingFile, () => fm);
    } else {
      const file = await this.plugin.entityService.createEntity(this.plugin.settings.paths.people, name, fm);
