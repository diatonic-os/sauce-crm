...
export class IntroModal extends Modal {
  ...
  onOpen(): void {
    ...
    const aRow = new Setting(contentEl).setName("Person A").addButton((b) => b
      .setButtonText("Pick A")
      .onClick(() => new WikilinkSuggest(this.app, [this.plugin.settings.paths.people], (_f, raw) => {
        this.a = raw; b.setButtonText(raw); this.recompute();
      }, false).open()));
    void aRow;

    new Setting(contentEl).setName("Person B").addButton((b) => b
      .setButtonText("Pick B")
      .onClick(() => new WikilinkSuggest(this.app, [this.plugin.settings.paths.people], (_f, raw) => {
        this.b = raw; b.setButtonText(raw); this.recompute();
      }, false).open()));

    this.result = contentEl.createDiv({ cls: "sauce-intro-result" });

    const btns = contentEl.createDiv({ cls: "sauce-buttons" });
    btns.createEl("button", { text: "Promote", cls: "sauce-button" }).onclick = () => this.promote();
  ...
  private recompute(): void {
    ...
    if (!aFile || !bFile) { this.result.setText("could not resolve both people"); return; }
    ...
    this.result.empty();
    this.result.createEl("p", { text: `Density: ${(r.score * 100).toFixed(1)}%` });
    this.result.createEl("p", { text: r.passes_threshold ? "✓ Clears ρ_adm threshold" : `Below threshold — missing: ${r.missing_for_threshold.join(", ")}` });
  }

  private async promote(): Promise<void> {
    if (!this.a || !this.b) { new Notice("Pick both A and B"); return; }
    const aFile = this.app.metadataCache.getFirstLinkpathDest(this.a, "");
    if (!aFile) { new Notice("could not resolve A"); return; }
