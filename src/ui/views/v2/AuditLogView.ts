...
export class AuditLogView extends ItemView {
  ...
  getViewType(): string { return VIEW_AUDIT_LOG; }
  getDisplayText(): string { return "Sauce: Audit Log"; }
  ...
  private async render(): Promise<void> {
    ...
    const verifyBtn = toolbar.createEl("button", { cls: "sauce-button sauce-button-secondary", text: "Verify Chain" });
    verifyBtn.onclick = async () => {
      try {
        const fn = (audit as unknown as { verifyChain?: () => Promise<{ ok: boolean; brokenAt: number | null }> }).verifyChain;
    ...
    try {
      ...
      root.createDiv({ cls: "sauce-error", text: `Audit query failed: ${msg}` });
      return;
    }
    ...
    for (const h of ["ts", "op", "entityId", "agentId", "integration", "signature"]) {
    ...
    for (const raw of rows) {
      const r = raw as {
        ...
        integration?: string | null; signature?: string;
      };
      const tr = body.createEl("tr");
