...
interface Plot { x: number; y: number; basename: string; lat: number; lon: number; kind: "person" | "org"; }
...
export class MapView extends ItemView {
  ...
  getViewType(): string { return VIEW_MAP; }
  getDisplayText(): string { return "Sauce: Map"; }
  getIcon(): string { return "map"; }

  async onOpen(): Promise<void> {
    const root = this.contentEl;
    root.empty(); root.addClass("sauce-view"); root.addClass("sauce-map");
    root.createEl("h2", { text: "Map" });
    const note = root.createEl("p", { cls: "sauce-view-desc" });

    this.canvas = root.createEl("canvas", { cls: "sauce-map-canvas" }) as HTMLCanvasElement;
    ...
    if (points.length === 0) {
      note.setText("No entities with lat/lon yet. Use the geocode skill on a person/org to populate.");
      return;
    }
  ...
  private collect(): Plot[] {
    ...
    for (const p of this.plugin.entityService.allPeople()) {
      const lat = Number((p.frontmatter as any).lat), lon = Number((p.frontmatter as any).lon);
      if (Number.isFinite(lat) && Number.isFinite(lon)) out.push({ x: 0, y: 0, basename: p.file.basename, lat, lon, kind: "person" });
    }
    for (const o of this.plugin.entityService.allOrgs()) {
      const lat = Number((o.frontmatter as any).lat), lon = Number((o.frontmatter as any).lon);
      if (Number.isFinite(lat) && Number.isFinite(lon)) out.push({ x: 0, y: 0, basename: o.file.basename, lat, lon, kind: "org" });
    }
    return out;
  }
  ...
  private draw(points: Plot[]): void {
    ...
    if (maxLat - minLat < 0.001) { maxLat += 0.05; minLat -= 0.05; }
    if (maxLon - minLon < 0.001) { maxLon += 0.05; minLon -= 0.05; }
    const pad = 30;
    ...
    this.plots = points.map((p) => {
      ...
      const y = pad + (1 - (p.lat - minLat) / (maxLat - minLat)) * (h - 2 * pad);
      return { ...p, x, y };
    });

    for (const pl of this.plots) {
      ctx.fillStyle = pl.kind === "org" ? "rgba(220,160,80,0.85)" : "rgba(80,160,220,0.85)";
  ...
  private openEntity(basename: string): void {
    ...
    if (!f) { new Notice(`could not resolve ${basename}`); return; }
