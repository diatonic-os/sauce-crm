...
export async function renderPluginConfigBlock(el: HTMLElement, plugin: SauceGraphPlugin): Promise<void> {
  el.empty();
  el.addClass("sauce-cp-suggestions");
  const svc = plugin.pluginConfig;
  if (!svc) { el.createEl("p", { cls: "sauce-cp-empty", text: "Plugin auto-config not initialized." }); return; }

  const head = el.createDiv({ cls: "sauce-cp-sec" });
  head.createEl("h4", { cls: "sauce-cp-sec-title", text: "Plugin auto-configuration" });
  head.createEl("p", { cls: "sauce-cp-empty", text: "Sauce proposes canonical settings for supported plugins; nothing is written until you Apply. Changes back up + trace first, and take effect after reloading the plugin." });
  ...
  const render = async (): Promise<void> => {
    ...
    for (const st of rows) {
      ...
      main.createEl("div", { cls: "sauce-cp-card-title", text: `${st.profile.label}  (${st.profile.kind})` });
      const sub = STATE_LABEL[st.state] + (st.changes.length ? ` · ${st.changes.length} setting(s) to apply` : "");
      main.createEl("div", { cls: "sauce-cp-card-sub", text: sub });
      if (st.state === "installed" || st.state === "drift") {
        anyActionable = true;
        const apply = card.createEl("button", { cls: "sauce-button sauce-button-secondary" });
        apply.setText("Apply");
        apply.onclick = async () => {
          try {
          ...
          } catch (e) { new Notice(`Apply failed: ${e instanceof Error ? e.message : String(e)}`); }
          await render();
        };
      }
    }
    applyAll.disabled = !anyActionable;
    if (!rows.length) body.createEl("p", { cls: "sauce-cp-empty", text: "No supported plugins configured." });
  };

  applyAll.onclick = async () => {
    try {
      const summary = await svc.applyAll();
      new Notice(summary.length
        ? `Auto-configured ${summary.length} plugin(s). Reload them to take effect.`
        : "All supported plugins already match canon.");
    } catch (e) { new Notice(`Apply-all failed: ${e instanceof Error ? e.message : String(e)}`); }
    await render();
  };
