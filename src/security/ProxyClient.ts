...
export interface ProxyConfig { enabled: boolean; baseUrl: string; sharedSecret: string; }
...
export class ProxyClient {
  constructor(private readonly host: ProxyHost, private cfg: ProxyConfig) {}

  setConfig(cfg: ProxyConfig): void { this.cfg = cfg; }
  isEnabled(): boolean { return this.cfg.enabled && !!this.cfg.baseUrl && !!this.cfg.sharedSecret; }
  ...
  async fetch(url: string, init: { method?: string; headers?: Record<string, string>; body?: string } = {}): Promise<{ status: number; headers: Record<string, string>; body: string }> {
    ...
    if (!this.isEnabled()) {
      return this.host.fetch(url, { method, headers: init.headers ?? {}, body: body || undefined });
    }
    ...
    return this.host.fetch(this.cfg.baseUrl, { method, headers, body: body || undefined });
  }
}
