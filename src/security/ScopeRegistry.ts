...
export class ScopeNotGranted extends Error {
  constructor(public readonly integration: string, public readonly scope: string) {
...
export class ScopeRegistry {
  ...
  load(map: ScopeMap): void { this.scopes = JSON.parse(JSON.stringify(map)); }
  toJSON(): ScopeMap { return JSON.parse(JSON.stringify(this.scopes)); }
  ...
  check(integration: string, scope: string): boolean { return !!this.scopes[integration]?.[scope]; }

  require(integration: string, scope: string): void {
    if (!this.check(integration, scope)) throw new ScopeNotGranted(integration, scope);
  }

  list(integration: string): Record<string, boolean> { return { ...(this.scopes[integration] ?? {}) }; }
  integrations(): string[] { return Object.keys(this.scopes); }
}

export const DEFAULT_SCOPES: ScopeMap = {
  google_workspace: {
    'calendar.read': true, 'calendar.write': false,
    'gmail.read': true, 'gmail.modify': false, 'gmail.send': false,
    'drive.read': true, 'drive.write': false, 'contacts.read': true,
  },
  microsoft_365: {
    'calendar.read': true, 'calendar.write': false,
    'mail.read': true, 'mail.modify': false, 'mail.send': false,
    'files.read': true, 'files.write': false, 'contacts.read': true,
  },
