...
export interface OAuthHost {
  ...
  fetchJson<T>(url: string, init?: { method?: string; headers?: Record<string, string>; body?: string }): Promise<T>;
}
...
export class OAuthFlow {
  ...
  constructor(private readonly host: OAuthHost, private readonly vault: KeyVault, private readonly subtle: SubtleCrypto, private readonly random: (n: number) => Uint8Array) {}

  registerProvider(id: ProviderId, cfg: OAuthProviderConfig): void { this.providers.set(id, cfg); }
  ...
  async authorize(provider: ProviderId, scopes: string[]): Promise<TokenSet> {
    ...
    const params = new URLSearchParams({
      ...
      scope: effective.join(' '), state, code_challenge: challenge, code_challenge_method: 'S256',
    });
    ...
    if (cbUrl.searchParams.get('state') !== state) throw new Error('state mismatch (possible CSRF)');
    const code = cbUrl.searchParams.get('code');
    if (!code) throw new Error('authorization denied');

    const tokenBody = new URLSearchParams({
      grant_type: 'authorization_code', code, redirect_uri: redirectUri,
      code_verifier: verifier, client_id: cfg.clientId,
    });
    ...
    if (ts.refreshToken) await this.vault.put(`oauth:${provider}:refresh`, ts.refreshToken);
  ...
  async refresh(provider: ProviderId): Promise<TokenSet> {
    ...
    type R = { access_token: string; refresh_token?: string; expires_in?: number; scope?: string };
    ...
    if (r.refresh_token) await this.vault.put(`oauth:${provider}:refresh`, r.refresh_token);
    this.inMemoryTokens.set(provider, ts);
    return ts;
  }
  ...
  scopesGranted(provider: ProviderId): string[] { return this.inMemoryTokens.get(provider)?.scopes ?? []; }
  current(provider: ProviderId): TokenSet | null { return this.inMemoryTokens.get(provider) ?? null; }
}
