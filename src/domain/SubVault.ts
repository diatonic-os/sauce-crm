...
export class SubVault extends Entity {
  ...
  get role(): string {
    return this.frontmatter.role ?? "secondary";
  }
  get enabled(): boolean {
    return Boolean(this.frontmatter.enabled ?? true);
  }
  get spec_version(): string {
    return this.frontmatter.spec_version ?? "unknown";
  }
  get contract_ref(): string | null {
    return this.frontmatter.contract_ref ?? null;
  }
}
