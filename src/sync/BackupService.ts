...
export class BackupService {
  constructor(public app: App, public entities: EntityService, public query: QueryService) {}

  async run(): Promise<BackupReport> {
    const ts = Date.now();
    const stamp = new Date(ts).toISOString().replace(/[:.]/g, "-");
    const folder = "_backups";
    if (!this.app.vault.getAbstractFileByPath(folder)) await this.app.vault.createFolder(folder).catch(() => { /* ok */ });
    ...
    return { ts, path, entities: people.length + orgs.length + touches.length + addenda.length, edges: adjacency.length, bytes: content.length };
  }
  ...
  async prune(keep = 14): Promise<number> {
    ...
    if (!folder) return 0;
    const files = this.app.vault.getMarkdownFiles().concat(this.app.vault.getFiles().filter((f) => f.path.startsWith("_backups/") && f.path.endsWith(".json")));
    const backups = files.filter((f) => f.path.startsWith("_backups/sauce-backup-") && f.path.endsWith(".json"));
