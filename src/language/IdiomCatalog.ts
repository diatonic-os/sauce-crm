...
export interface Idiom { pattern: RegExp; rewrite: (m: RegExpMatchArray) => string; }

const TODAY = () => new Date().toISOString().slice(0, 10);
const ADD_DAYS = (n: number) => new Date(Date.now() + n * 86400000).toISOString().slice(0, 10);
...
export class IdiomCatalog {
  private idioms: Idiom[] = [
    ...
    { pattern: /^follow up with (.+?) in (\d+) days$/i, rewrite: (m) => `- [ ] follow-up ${m[1]} 📅 ${ADD_DAYS(parseInt(m[2], 10))}` },
  ];

  add(i: Idiom): void { this.idioms.push(i); }
  remove(index: number): void { this.idioms.splice(index, 1); }
  list(): Idiom[] { return [...this.idioms]; }
