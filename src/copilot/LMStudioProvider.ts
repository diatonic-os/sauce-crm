...
export class LMStudioProvider implements ICopilotProvider {
  ...
  async refreshModels(): Promise<void> {
    const r = await this.host
      .fetch(`${this.cfg.endpoint.replace(/\/$/, "")}/models`, {
        method: "GET",
  ...
  async ping(): Promise<{ ok: boolean; latencyMs: number; error?: string }> {
    ...
    try {
      const r = await this.host.fetch(
        `${this.cfg.endpoint.replace(/\/$/, "")}/models`,
  ...
  async *complete(req: CompletionRequest): AsyncIterable<CompletionEvent> {
    ...
    if (req.stream && this.host.fetchStream) {
      ...
      (body as Record<string, unknown>).stream_options = {
        include_usage: true,
      };
      try {
        const resp = await this.host.fetchStream(
          `${this.cfg.endpoint.replace(/\/$/, "")}/chat/completions`,
          {
            method: "POST",
    ...
    const resp = await this.host.fetch(
      `${this.cfg.endpoint.replace(/\/$/, "")}/chat/completions`,
      {
        method: "POST",
        headers: this.headers(),
  ...
  async embed(text: string, model: string): Promise<Float32Array> {
    const r = await this.host.fetch(
      `${this.cfg.endpoint.replace(/\/$/, "")}/embeddings`,
      {
        method: "POST",
        headers: this.headers(),
