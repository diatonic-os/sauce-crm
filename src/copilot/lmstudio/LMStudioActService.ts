...
export class LMStudioActService {
  constructor(
    ...
    private readonly sdkExports: { tool: (def: unknown) => unknown },
  ) {}
  ...
  async act(req: ActRequest): Promise<ActResult> {
    ...
    if (!model.act)
      throw new Error(
        "LM Studio model does not expose .act() — upgrade @lmstudio/sdk",
      );

    const calls: ActResult["toolCalls"] = [];
    const sdkTools = req.tools.map((t) =>
      this.sdkExports.tool({
        name: t.name,
    ...
    const opts: LMStudioActOpts = {
      ...
      onMessage: (m) => {
        ...
        if (m && typeof m === "object" && "getText" in m) {
          try {
            finalMessage = (m as { getText: () => string }).getText();
          } catch {
            /* */
          }
        }
