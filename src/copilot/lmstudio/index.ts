...
export {
  LMStudioChatService,
  type ChatMessage as LMStudioChatMessage,
  type ChatRequest as LMStudioChatRequest,
  type ChatStreamEvent as LMStudioChatStreamEvent,
  type ChatResult as LMStudioChatResult,
} from "./LMStudioChatService";
export {
  LMStudioActService,
  type ToolSpec,
  type ActRequest,
  type ActResult,
} from "./LMStudioActService";
...
export async function buildLMStudioIntegration(
  ...
  const sdkExports = opts.sdkLoader
    ...
    : ((
        (typeof require !== "undefined" ? require : null) as
          | null
          | ((m: string) => unknown)
      )?.("@lmstudio/sdk") as { tool: (def: unknown) => unknown });
  return {
    factory,
    client,
