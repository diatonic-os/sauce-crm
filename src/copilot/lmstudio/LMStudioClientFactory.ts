...
export interface LMStudioLlmNamespace {
  ...
  load(opts: { model: string; ttl?: number; signal?: AbortSignal; config?: Record<string, unknown> }): Promise<LMStudioLlmHandle>;
...
export interface LMStudioSystemNamespace {
  listDownloadedModels(): Promise<Array<{ modelKey?: string; path?: string; type?: string; sizeBytes?: number }>>;
  getLMStudioVersion?(): Promise<{ version: string; build?: string }>;
}

export interface LMStudioLlmHandle {
  readonly identifier?: string;
  respond(chat: unknown, opts?: LMStudioRespondOpts): Promise<{ content: string; stats?: LMStudioStats; nonReasoningContent?: string; reasoningContent?: string }>;
  complete?(prompt: string, opts?: LMStudioCompleteOpts): Promise<{ content: string; stats?: LMStudioStats }>;
  act?(chat: unknown, tools: unknown[], opts?: LMStudioActOpts): Promise<{ stats?: LMStudioStats; rounds?: unknown[] }>;
  unload(): Promise<void>;
...
export interface LMStudioEmbedHandle {
  embed(input: string | string[]): Promise<{ embedding: number[] } | { embedding: number[] }[]>;
  getContextLength(): Promise<number>;
  tokenize(input: string): Promise<number[]>;
  countTokens(input: string): Promise<number>;
  unload(): Promise<void>;
...
export interface LMStudioActOpts {
  ...
  guardToolCall?: (roundIndex: number, callId: string, ctx: { toolCallRequest: { name: string; arguments: Record<string, unknown> }; allow: () => void; deny: (reason: string) => void }) => void | Promise<void>;
...
export class LMStudioClientFactory {
  ...
  async build(): Promise<LMStudioClientLike> {
    ...
    if (!sdk?.LMStudioClient) throw new Error('@lmstudio/sdk not available — install with `npm install @lmstudio/sdk`');
    ...
    if (creds.apiToken) attempts.push({ ...baseOpts, apiToken: creds.apiToken });
    ...
    for (const opts of attempts) {
      try {
        ...
        if (!/Unrecognized key|Invalid parameter|Unknown option/i.test(msg)) throw e;
      }
    }
    throw lastError instanceof Error ? lastError : new Error('LMStudioClient construction failed');
  }

  private loadSdk(): { LMStudioClient: new (opts: Record<string, unknown>) => LMStudioClientLike } | null {
    if (this.options.sdkLoader) return this.options.sdkLoader() as { LMStudioClient: new (opts: Record<string, unknown>) => LMStudioClientLike };
    const req = (typeof require !== 'undefined' ? require : null) as null | ((m: string) => unknown);
    if (!req) return null;
    try { return req('@lmstudio/sdk') as { LMStudioClient: new (opts: Record<string, unknown>) => LMStudioClientLike }; }
    catch { return null; }
  }
}
