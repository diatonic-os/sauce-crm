...
describe("BridgeService (default-off, gated)", () => {
  ...
  it("does not start when disabled", async () => {
    svc = new BridgeService({ backend, crypto, lanceStatus, discover: async () => "100.64.0.1" });
    const st = await svc.start({ enabled: false, port: 0, bindHost: "", pairingToken: "tok" });
    expect(st).toEqual({ running: false, reason: "disabled" });
  });

  it("refuses to start without a pairing token", async () => {
    svc = new BridgeService({ backend, crypto, lanceStatus, discover: async () => "100.64.0.1" });
    const st = await svc.start({ enabled: true, port: 0, bindHost: "", pairingToken: "" });
    expect(st).toEqual({ running: false, reason: "not-paired" });
  });

  it("refuses to start when no Tailscale address can be resolved", async () => {
    svc = new BridgeService({ backend, crypto, lanceStatus, discover: async () => null });
    const st = await svc.start({ enabled: true, port: 0, bindHost: "", pairingToken: "tok" });
  ...
  it("starts on an explicit bindHost when enabled + paired, and stops", async () => {
    ...
    const st = await svc.start({ enabled: true, port: 0, bindHost: "127.0.0.1", pairingToken: "tok" });
  ...
  it("auto-discovers the bind address when not overridden", async () => {
    svc = new BridgeService({ backend, crypto, lanceStatus, discover: async () => "127.0.0.1" });
    const st = await svc.start({ enabled: true, port: 0, bindHost: "", pairingToken: "tok" });
    expect(st.running).toBe(true);
    await svc.stop();
  });
