...
export interface TouchRecord { id: string; date: string; attendees: string[]; outcomeTags?: string[]; }
export interface EdgeProposal { fromId: string; toId: string; edgeType: 'knows' | 'worked_with'; confidence: number; verdict: Verdict; sources: string[]; }
...
export class EdgeInferrer {
  ...
  inferFrom(touches: TouchRecord[]): EdgeProposal[] {
    const pairs = new Map<string, { count: number; lastTs: number; sources: Set<string>; advice: number; intro: number }>();
    for (const t of touches) {
      ...
      for (let i = 0; i < t.attendees.length; i++) {
        for (let j = i + 1; j < t.attendees.length; j++) {
          ...
          const k = `${a}|${b}`;
          const rec = pairs.get(k) ?? { count: 0, lastTs: 0, sources: new Set<string>(), advice: 0, intro: 0 };
          rec.count += 1;
