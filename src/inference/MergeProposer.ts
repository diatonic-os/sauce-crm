...
export interface MergeCandidate { entityType: 'person' | 'org'; aId: string; bId: string; confidence: number; verdict: Verdict; reason: string[]; }
export interface CandidateRecord { id: string; name: string; emails: string[]; phones: string[]; type: 'person' | 'org'; }
    ...
    for (const [, group] of byNorm) {
      for (let i = 0; i < group.length; i++) {
        for (let j = i + 1; j < group.length; j++) {
          ...
          let emailMatch = 0;
          for (const e of a.emails) if (b.emails.includes(e)) { emailMatch = 1; reasons.push('email_match'); break; }
