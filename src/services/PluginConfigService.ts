...
export interface PluginConfigHost {
  ...
  readConfig(id: string, kind: PluginKind): Promise<Record<string, unknown> | null>;
  writeConfig(id: string, kind: PluginKind, data: Record<string, unknown>): Promise<void>;
  /** Persist a backup of the pre-write config (e.g. to the host backup target). */
  backupConfig(id: string, kind: PluginKind, data: Record<string, unknown> | null): Promise<void>;
}
...
export function defaultProfiles(): CanonicalProfile[] {
  return [
    { id: "daily-notes", kind: "core", label: "Daily Notes", settings: { folder: "_events/daily", format: "YYYY-MM-DD", template: "_templates/daily.md" } },
    { id: "templates", kind: "core", label: "Templates", settings: { folder: "_templates" } },
    { id: "obsidian-tasks-plugin", kind: "community", label: "Tasks", settings: { globalFilter: "", setDoneDate: true, autoSuggestInEditor: true } },
    { id: "dataview", kind: "community", label: "Dataview", settings: { enableDataviewJs: false, refreshEnabled: true } },
    { id: "templater-obsidian", kind: "community", label: "Templater", settings: { templates_folder: "_templates", trigger_on_file_creation: false } },
    { id: "calendar", kind: "community", label: "Calendar", settings: { weekStart: "locale", showWeeklyNote: false } },
  ];
}
...
export class PluginConfigService {
  ...
  list(): CanonicalProfile[] { return this.profiles; }
  ...
  async status(profile: CanonicalProfile): Promise<PluginConfigStatus> {
    ...
    const current = (await this.host.readConfig(profile.id, profile.kind)) ?? {};
    const changes: ConfigChange[] = [];
    for (const [key, want] of Object.entries(profile.settings)) {
      if (!shallowEqual(current[key], want)) changes.push({ key, from: current[key], to: want });
    }
  ...
  async apply(profile: CanonicalProfile): Promise<ConfigChange[]> {
    ...
    await this.trace?.record("plugin-config", `plugin:${profile.id}`, "config",
      JSON.stringify({ applied: st.changes }), { meta: { kind: profile.kind, keys: st.changes.map((c) => c.key) } })
      .catch?.(() => {});
    return st.changes;
  }
