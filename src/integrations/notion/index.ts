...
export class NotionIntegration implements IIntegration {
  ...
  client(): NotionClient | null {
    ...
    if (!this._client) this._client = new NotionClient({ fetch: this.host.fetch, token: this.host.token });
    return this._client;
  }
  ...
  conflictFields(local: Record<string, unknown>, page: { properties: Record<string, any> }, fields: string[]): { name: string; local: unknown; remote: unknown }[] {
    ...
    for (const f of fields) {
      ...
      if (!shallowEqual(localVal, remote)) out.push({ name: f, local: localVal, remote });
    }
    return out;
  }
}
...
function extractNotionProp(p: any): unknown {
  if (!p) return null;
  switch (p.type) {
    case "title":      return (p.title ?? []).map((x: any) => x.plain_text).join("");
    case "rich_text":  return (p.rich_text ?? []).map((x: any) => x.plain_text).join("");
