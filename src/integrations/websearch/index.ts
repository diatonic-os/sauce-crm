...
export interface WebSearchHost {
  ...
  markdownExtract(html: string): string;
}
...
export class BraveSearch implements IWebSearchProvider {
  ...
  async search(q: string, opts: SearchOpts): Promise<SearchResult[]> {
    ...
    const r = await this.host.proxy.fetch(url, {
      method: "GET",
    ...
    for (const it of results) {
      out.push({
        ...
        publishedTs: null,
        fetchedTs: now,
        hash: await this.host.sha256Hex(it.url),
      });
    }
    return out;
  }
  ...
  async search(q: string, opts: SearchOpts): Promise<SearchResult[]> {
    ...
    for (const it of j.results) {
      out.push({
        ...
        title: it.title,
        snippet: it.content,
        publishedTs: it.published_date ? Date.parse(it.published_date) : null,
        fetchedTs: now,
        hash: await this.host.sha256Hex(it.url),
      });
    }
  ...
  async search(q: string, opts: SearchOpts): Promise<SearchResult[]> {
    ...
    for (const it of j.results.slice(0, opts.count ?? 10)) {
      out.push({
        ...
        title: it.title,
        snippet: it.content,
        publishedTs: it.publishedDate ? Date.parse(it.publishedDate) : null,
        fetchedTs: now,
        hash: await this.host.sha256Hex(it.url),
      });
    }
