...
export async function graphGet<T>(opts: GraphSubClientOpts, path: string, params?: Record<string, string | number | undefined>): Promise<T> {
  ...
  if (r.status < 200 || r.status >= 300) throw new Error(`graph api ${r.status}: ${r.body.slice(0, 200)}`);
