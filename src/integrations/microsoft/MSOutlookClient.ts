...
export class MSOutlookClient {
  ...
  async listMessages(params: { filter?: string; top?: number; orderBy?: string } = {}): Promise<GraphMessage[]> {
    const r = await graphGet<{ value?: GraphMessage[] }>(this.opts, "/me/messages", {
      $top: params.top ?? 50,
  ...
  async getMessage(id: string): Promise<GraphMessage & { body?: { contentType?: string; content?: string } }> {
