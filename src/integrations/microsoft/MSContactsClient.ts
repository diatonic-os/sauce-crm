...
export class MSContactsClient {
  ...
  async listContacts(params: { top?: number; skip?: number } = {}): Promise<GraphContact[]> {
    const r = await graphGet<{ value?: GraphContact[] }>(this.opts, "/me/contacts", {
      $top: params.top ?? 100, $skip: params.skip,
