...
export class GContactsClient {
  ...
  async listConnections(
    params: { pageSize?: number; pageToken?: string; syncToken?: string } = {},
  ): Promise<{
    connections: GContact[];
    ...
    const r = await googleGetJson<{
      ...
      nextPageToken?: string;
      nextSyncToken?: string;
    }>(this.opts, "/people/v1/people/me/connections", {
  ...
  async search(query: string, pageSize = 25): Promise<GContact[]> {
    const r = await googleGetJson<{ results?: { person: GContact }[] }>(
      this.opts,
      "/people/v1/people:searchContacts",
      {
        query,
        pageSize,
