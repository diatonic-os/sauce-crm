...
export type { TwilioCall, TwilioMessage, TwilioRecording, TwilioTranscription, TwilioAuth } from './TwilioClient';
...
export class TwilioIntegration implements IIntegration {
  ...
  client(): TwilioClient | null {
    ...
    if (!this._client) this._client = new TwilioClient({ fetch: this.host.fetch, auth: this.host.auth });
    return this._client;
  }
  ...
  async syncResource(id: string): Promise<{ pulled: number; pushed: number; errors: number }> {
    ...
    try {
      ...
      else if (id === "messages") pulled = (await c.listMessages({ pageSize: 50 })).length;
      else if (id === "recordings") pulled = (await c.listRecordings()).length;
      else if (id === "transcriptions") pulled = (await c.listTranscriptions()).length;
    } catch { errors++; }
