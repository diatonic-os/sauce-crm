...
export interface TwilioAuth { accountSid: string; authToken: string; }
...
export interface TwilioCall {
  sid: string; from: string; to: string; status: string; direction: string;
  duration?: string; startTime?: string; endTime?: string;
}

export interface TwilioMessage {
  sid: string; from: string; to: string; body: string; status: string; direction: string;
  dateCreated?: string; dateSent?: string;
}

export interface TwilioRecording {
  sid: string; callSid?: string; duration?: string; channels?: number;
  status?: string; uri?: string; mediaUrl?: string; dateCreated?: string;
}

export interface TwilioTranscription {
  sid: string; recordingSid?: string; transcriptionText?: string;
  status?: string; price?: string; dateCreated?: string;
}
...
export class TwilioClient {
  ...
  private base(): string { return this.opts.base ?? "https://api.twilio.com/2010-04-01"; }
  ...
  private async get<T>(path: string, params?: Record<string, string | number | undefined>): Promise<T> {
    ...
    const r = await this.opts.fetch.fetch(`${this.base()}/Accounts/${a.accountSid}${path}.json${qs}`, {
      method: "GET",
      headers: { Authorization: basicAuthHeader(a), Accept: "application/json" },
    });
    if (r.status < 200 || r.status >= 300) throw new Error(`twilio api ${r.status}: ${r.body.slice(0, 200)}`);
    return JSON.parse(r.body) as T;
  }

  async listCalls(params: { from?: string; to?: string; status?: string; pageSize?: number } = {}): Promise<TwilioCall[]> {
    const r = await this.get<{ calls?: any[] }>("/Calls", { From: params.from, To: params.to, Status: params.status, PageSize: params.pageSize ?? 50 });
    return (r.calls ?? []).map(decodeCall);
  }

  async listMessages(params: { from?: string; to?: string; pageSize?: number } = {}): Promise<TwilioMessage[]> {
    const r = await this.get<{ messages?: any[] }>("/Messages", { From: params.from, To: params.to, PageSize: params.pageSize ?? 50 });
    return (r.messages ?? []).map(decodeMessage);
  }

  async listRecordings(callSid?: string): Promise<TwilioRecording[]> {
    const r = await this.get<{ recordings?: any[] }>("/Recordings", { CallSid: callSid, PageSize: 50 });
    return (r.recordings ?? []).map(decodeRecording);
  }

  async listTranscriptions(): Promise<TwilioTranscription[]> {
    const r = await this.get<{ transcriptions?: any[] }>("/Transcriptions", { PageSize: 50 });
    return (r.transcriptions ?? []).map(decodeTranscription);
  }

  /** Resolve full media URL for a recording (raw audio). */
  async recordingMediaUrl(recordingSid: string, format: "wav" | "mp3" = "mp3"): Promise<string> {
    const a = await this.opts.auth();
