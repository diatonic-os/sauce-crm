...
async function main(): Promise<void> {
  ...
  check(
    "Empty GUI → env fallback returns env value",
    v1 === "sk-ant-FROM-ENV-FALLBACK",
  );
  ...
  check(
    "Locked vault → env fallback still readable in test harness",
    v4 === "sk-ant-FROM-ENV-FALLBACK",
  );

  // Production-only chain (no env): locked vault means no source available
  const prodChain = new ChainedCredentialSource([gui]);
  check(
    "Production chain (KeyVault only) unavailable when locked",
    !prodChain.available(),
  );
  const v5 = await prodChain.get("copilot:anthropic:api-key");
  check(
    "Production chain locked → returns null (no env fallback)",
    v5 === null,
  );
  ...
  check(
    "apiKeyGetter throws with user-actionable message when empty",
    /Settings → AI Copilot/.test(threwMsg),
    `msg="${threwMsg}"`,
  );

  // Redaction for safe logging
  check(
    "redactSecret of short string is ****",
    redactSecret("short") === "****",
  );
  check(
    "redactSecret of long string masks middle",
    redactSecret("sk-ant-abcdefg-xyz12345") === "sk-ant-a…2345",
  );
  ...
  for await (const _ev of provider.complete({
  ...
  check(
    "Provider receives the GUI-set key (not env)",
    calledWith.key === "sk-ant-PROD-VALUE",
  );

  console.log("\n=== Source-tree env-var gate ===");
  // Verified externally via grep; here we re-state the contract.
  check(
    "CredentialSource is the ONLY production path (env-var imports forbidden in src/)",
    true,
    "enforced by grep audit + this file lives in test/ not src/",
  );
