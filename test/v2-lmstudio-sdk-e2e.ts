...
function redact(s: string | null | undefined): string { return s ? `${s.slice(0, 8)}…${s.slice(-4)}` : '(none)'; }
...
async function main(): Promise<void> {
  ...
  console.log('\n--- Model management ---');
  let downloaded: Awaited<ReturnType<typeof integration.models.listDownloaded>> = [];
  try {
  ...
  } catch (e) { check('listDownloaded works', false, e instanceof Error ? e.message : String(e)); }

  try {
    const loaded = await integration.models.listLoaded();
    check('listLoaded works', Array.isArray(loaded), `loaded=${loaded.length}`);
  } catch (e) { check('listLoaded works', false, e instanceof Error ? e.message : String(e)); }

  const ver = await integration.models.lmStudioVersion();
  if (ver) check('lmStudioVersion returns build info', !!ver.version, `v=${ver.version}`);
  else skipMsg('lmStudioVersion', 'SDK build does not expose getLMStudioVersion');
  ...
  const chatCandidates = downloaded
    .map((m) => m.modelKey)
    .filter((k) => !k.toLowerCase().includes('embed') && !k.toLowerCase().includes('embedding'))
    .filter((k) => !!k);
  ...
  if (!targetModel) {
    ...
    try {
      ...
      let cancelled = false;
      let returned = false;
      try { await cancellable; returned = true; }
      ...
      if (embedCandidates.length === 0) skipMsg('embed', 'no embedding model downloaded');
      else {
        ...
        try {
          ...
          check('embedding is non-trivial', Array.from(v).some((x) => Math.abs(x) > 0.001));
        } catch (e) { check('embed live', false, e instanceof Error ? e.message : String(e)); }
      }
      ...
      try {
        ...
        if (!toolHandle.act) {
          ...
          const actResult = await integration.act.act({
            ...
            tools: [{
              ...
              invoke: async (args) => { invocations += 1; return Number(args.a) + Number(args.b); },
            }],
          });
        ...
        if (/does not support tool use|not supported|tool_use/i.test(msg)) skipMsg('act() agentic', 'model does not support tool use');
