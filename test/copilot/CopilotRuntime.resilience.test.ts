...
class FakeProvider implements ISauceBotProvider {
  ...
  async ping(): Promise<{ ok: boolean; error?: string }> {
    const p = this.opts.pings?.[Math.min(this.pingCalls, (this.opts.pings.length ?? 1) - 1)];
    this.pingCalls++;
    return p ?? { ok: true };
  }
  async *complete(_req: CompletionRequest): AsyncIterable<CompletionEvent> {
    const round = this.opts.rounds?.[Math.min(this.completeCalls, (this.opts.rounds.length ?? 1) - 1)] ?? [
...
describe("SauceBotRuntime — resilience / health", () => {
  it("short-circuits to a clear done:error when an unreachable provider's ping fails", async () => {
    ...
    const done = events.find((e) => e.type === "done") as { reason: string; error?: string };
  ...
  it("retries a transient ping failure, then succeeds", async () => {
    const provider = new FakeProvider({
      ...
      rounds: [[{ type: "text", delta: "PONG" }, { type: "done", reason: "end_turn" }]],
    });
    const rt = makeRuntime(provider, 2);
    const events = await collect(rt);
    expect(provider.pingCalls).toBe(2);
    expect(events.some((e) => e.type === "status" && e.state === "retrying")).toBe(true);
    expect(events.some((e) => e.type === "text" && e.delta === "PONG")).toBe(true);
  ...
  it("emits connecting/loading status and passes reasoning through", async () => {
    ...
    expect(events.some((e) => e.type === "status" && e.state === "loading")).toBe(true);
    expect(events.some((e) => e.type === "reasoning" && e.delta === "thinking")).toBe(true);
  });
  ...
  it("salvages a reasoning-only turn via a reasoning-extraction pass", async () => {
    const provider = new FakeProvider({
      ...
      rounds: [
        ...
        [{ type: "reasoning", delta: "thinking hard…" }, { type: "done", reason: "end_turn" }],
        // Extraction call (completeOnce): now it produces the answer.
        [{ type: "text", delta: "FINAL ANSWER" }, { type: "done", reason: "end_turn" }],
      ],
    });
    const rt = makeRuntime(provider);
    const events = await collect(rt);
    expect(provider.completeCalls).toBe(2); // main turn + extraction
    expect(events.some((e) => e.type === "text" && e.delta === "FINAL ANSWER")).toBe(true);
  });
  ...
  it("falls back to surfacing the reasoning tail when extraction also yields nothing", async () => {
    const provider = new FakeProvider({
      pings: [{ ok: true }],
      rounds: [
        [{ type: "reasoning", delta: "deep thoughts about ranking" }, { type: "done", reason: "end_turn" }],
        [{ type: "done", reason: "end_turn" }], // extraction returns no text either
      ],
    });
    const rt = makeRuntime(provider);
    const events = await collect(rt);
    const salvaged = events.find((e) => e.type === "text") as { delta: string } | undefined;
