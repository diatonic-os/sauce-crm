...
describe("AnthropicProvider — SSE streaming", () => {
  ...
  function textStream(): string[] {
    const all =
      ...
      evt("content_block_delta", { index: 0, delta: { type: "text_delta", text: "Hel" } }) +
      evt("content_block_delta", { index: 0, delta: { type: "text_delta", text: "lo" } }) +
      evt("content_block_delta", { index: 0, delta: { type: "text_delta", text: " world" } }) +
      evt("content_block_stop", { index: 0 }) +
      evt("message_delta", { delta: { stop_reason: "end_turn" }, usage: { output_tokens: 3 } }) +
      evt("message_stop", {});
  ...
  it("emits text deltas, usage, and done from Anthropic SSE", async () => {
    ...
    const provider = new AnthropicProvider(host, async () => "sk-test");
    const events = await collect(provider.complete({
      model: "claude-opus-4-7", messages: [{ role: "user", content: "hi" }], stream: true,
    }));
  ...
  it("assembles streamed tool_use input_json_delta chunks", async () => {
    ...
    const all =
      ...
      evt("content_block_start", { index: 0, content_block: { type: "tool_use", id: "toolu_1", name: "log_touch" } }) +
      evt("content_block_delta", { index: 0, delta: { type: "input_json_delta", partial_json: '{"con' } }) +
      evt("content_block_delta", { index: 0, delta: { type: "input_json_delta", partial_json: 'tact":"alice"}' } }) +
      evt("content_block_stop", { index: 0 }) +
      evt("message_delta", { delta: { stop_reason: "tool_use" }, usage: { output_tokens: 9 } }) +
      evt("message_stop", {});
  ...
  it("yields done:error on 4xx without throwing", async () => {
    const host = new ProviderHostMock();
    host.routeStream("/messages", { status: 401, chunks: [JSON.stringify({ error: "unauth" })] });
    const provider = new AnthropicProvider(host, async () => "sk-test");
    const events = await collect(provider.complete({
      model: "claude-opus-4-7", messages: [{ role: "user", content: "hi" }], stream: true,
    }));
