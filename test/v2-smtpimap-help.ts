...
import {
  ...
  helpForEmail,
  helpById,
} from "../src/integrations/smtpimap/HelpLinks";
...
async function main(): Promise<void> {
  ...
  check(
    ...
    PROVIDER_HELP.length === 5,
    `n=${PROVIDER_HELP.length}`,
  );
  check(
    "Gmail matches user@gmail.com",
    helpForEmail("user@gmail.com")?.id === "google_workspace",
  );
  check(
    "iCloud matches @icloud.com",
    helpForEmail("test@icloud.com")?.id === "apple_icloud",
  );
  check(
    "Outlook matches @outlook.com",
    helpForEmail("foo@outlook.com")?.id === "microsoft_365",
  );
  check(
    "Fastmail matches @fastmail.com",
    helpForEmail("a@fastmail.com")?.id === "fastmail",
  );
  check(
    "Proton matches @proton.me",
    helpForEmail("x@proton.me")?.id === "protonmail",
  );
  ...
  for (const p of PROVIDER_HELP) {
    ...
    const okDomain =
      u.hostname.endsWith(p.domain) ||
      u.hostname.includes(p.domain.replace(/^.*\.([^.]+\.[^.]+)$/, "$1"));
    check(
      `${p.id} appPasswordUrl on first-party (${u.hostname})`,
      u.protocol === "https:" && okDomain,
    );
  }
  ...
  const page = new SmtpImapPage({
    ...
    saveSecret: async (k: string, v: string) => {
      saved[k] = v;
    },
  ...
  const helpButtons = buttons.filter((b) =>
    b.textContent.includes("app password"),
  );
  check(
    '5 "Open app password page" buttons rendered',
    helpButtons.length === 5,
    `n=${helpButtons.length}`,
  );
  // Click the first (Gmail) button
  helpButtons[0].click();
  check(
    "Clicking Gmail help opens myaccount.google.com",
    opened[0] === "https://myaccount.google.com/apppasswords",
    `opened=${opened[0]}`,
  );
  ...
  check(
    "OAuth setup buttons rendered for providers that support it",
    oauthButtons.length === 2,
    `n=${oauthButtons.length}`,
  );
