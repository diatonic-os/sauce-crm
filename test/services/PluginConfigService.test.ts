...
function fakeHost(opts: { installed?: boolean; config?: Record<string, unknown> | null } = {}) {
  ...
  const host: PluginConfigHost = {
    ...
    writeConfig: async (_id, _k, data) => { writes.push(data); config = data; },
    backupConfig: async (_id, _k, data) => { backups.push(data); },
  };
  return { host, writes, backups, get config() { return config; } };
}
...
describe("PluginConfigService.status", () => {
  ...
  it("reports 'installed' (unconfigured) when no canonical key is present", async () => {
    ...
    expect(st.changes).toEqual([{ key: "folder", from: undefined, to: "_templates" }]);
  });

  it("reports 'configured' when all canonical keys already match", async () => {
    const h = fakeHost({ installed: true, config: { folder: "_templates", other: 1 } });
    const svc = new PluginConfigService(h.host, [PROFILE]);
    const st = await svc.status(PROFILE);
  ...
  it("reports 'drift' when a canonical key is present but changed", async () => {
    const h = fakeHost({ installed: true, config: { folder: "OtherTemplates" } });
    const svc = new PluginConfigService(h.host, [PROFILE]);
    const st = await svc.status(PROFILE);
    expect(st.state).toBe("drift");
    expect(st.changes).toEqual([{ key: "folder", from: "OtherTemplates", to: "_templates" }]);
  });
});
...
describe("PluginConfigService.apply", () => {
  it("backs up, merges canonical over current (preserving other keys), writes, returns changes", async () => {
    const h = fakeHost({ installed: true, config: { folder: "Old", keepMe: 42 } });
    const svc = new PluginConfigService(h.host, [PROFILE]);
  ...
  it("records a provenance trace on apply", async () => {
    ...
    const trace = { record: async (op: string, subject: string) => { calls.push({ op, subject }); return {}; } };
    const svc = new PluginConfigService(h.host, [PROFILE], trace);
    await svc.apply(PROFILE);
    expect(calls).toEqual([{ op: "plugin-config", subject: "plugin:templates" }]);
  });
  ...
  it("applyAll returns only the plugins that needed changes", async () => {
    ...
    const svc = new PluginConfigService(installed.host, [
      PROFILE,
      { id: "dataview", kind: "community", label: "Dataview", settings: { refreshEnabled: true } },
    ]);
