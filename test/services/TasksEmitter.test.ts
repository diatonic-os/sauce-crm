...
describe("toCheckbox", () => {
  it("renders status, contact, priority, due, tags in Tasks-plugin format", () => {
    const t: SauceTask = { title: "Follow up", status: "todo", contact: "Bob Lee", priority: "high", due: "2026-06-01", tags: ["sauce/task", "vip"] };
    expect(toCheckbox(t)).toBe("- [ ] Follow up [[Bob Lee]] ⏫ 📅 2026-06-01 #sauce/task #vip");
  });
...
describe("parseCheckbox", () => {
  ...
  it("parses status, due, priority, contact, tags", () => {
    const t = parseCheckbox("- [/] Call [[Acme Corp]] 🔺 📅 2026-07-04 #followup");
    expect(t).toEqual({ title: "Call", status: "in_progress", due: "2026-07-04", priority: "urgent", contact: "Acme Corp", tags: ["followup"] });
  });
  ...
  it("round-trips a full task", () => {
    const t: SauceTask = { title: "Draft memo", status: "todo", contact: "Jane", priority: "medium", due: "2026-05-30", tags: ["q3"] };
    expect(parseCheckbox(toCheckbox(t))).toEqual(t);
  });

  it("handles a bare task with no metadata", () => {
    expect(parseCheckbox("- [ ] simple task")).toEqual({ title: "simple task", status: "todo" });
  });
});
