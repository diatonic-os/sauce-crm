...
describe("ObsidianPluginConfigHost", () => {
  ...
  it("detects core plugins via internalPlugins.config", () => {
    const { app } = fakeApp({
      coreConfig: { "daily-notes": true, templates: false },
    });
    const h = new ObsidianPluginConfigHost(app);
  ...
  it("reads community config from plugins/<id>/data.json", async () => {
    ...
    const h = new ObsidianPluginConfigHost(app);
    expect(await h.readConfig("dataview", "community")).toEqual({
      refreshEnabled: true,
    });
  ...
  it("reads core config from <id>.json", async () => {
    const { app } = fakeApp({
      files: { ".obsidian/daily-notes.json": JSON.stringify({ folder: "X" }) },
    });
    const h = new ObsidianPluginConfigHost(app);
  ...
  it("writes config to the correct path per kind", async () => {
    ...
    expect(JSON.parse(files[".obsidian/templates.json"])).toEqual({
      folder: "_templates",
    });
    expect(JSON.parse(files[".obsidian/plugins/dataview/data.json"])).toEqual({
      refreshEnabled: true,
    });
  });
  ...
  it("backs up into the vault backup dir (and skips null configs)", async () => {
    ...
    expect(
      Object.keys(files).some((p) => p.startsWith("_Plugin-Config/_backups")),
    ).toBe(false);
    await h.backupConfig("dataview", "community", { refreshEnabled: true });
    const backup = Object.keys(files).find((p) =>
      p.startsWith("_Plugin-Config/_backups/dataview."),
    );
