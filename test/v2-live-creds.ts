...
function redact(s: string | undefined): string { return s ? s.slice(0, 8) + '...' + s.slice(-4) : '(none)'; }
...
async function main(): Promise<void> {
  ...
  if (!env.ANTHROPIC_API_KEY) skipMsg('anthropic e2e', 'no ANTHROPIC_API_KEY');
    ...
    try {
      for await (const ev of prov.complete({
        ...
        if (ev.type === 'usage') { inTok = ev.inputTokens; outTok = ev.outputTokens; }
        if (ev.type === 'done') { doneReason = ev.reason; if (ev.reason === 'error' && (ev as { error?: string }).error) console.log(`    (provider error: ${(ev as { error?: string }).error?.slice(0, 200)})`); }
      }
    ...
    } catch (e) { check('Anthropic e2e', false, e instanceof Error ? e.message : String(e)); }
  }
  ...
  if (!env.OPENAI_API_KEY) skipMsg('openai e2e', 'no OPENAI_API_KEY');
    ...
    try {
      for await (const ev of prov.complete({
        ...
        if (ev.type === 'text') textSeen += ev.delta;
        if (ev.type === 'done' && ev.reason === 'error' && (ev as { error?: string }).error) console.log(`    (provider error: ${(ev as { error?: string }).error?.slice(0, 200)})`);
      }
      check('OpenAI chat returned text', textSeen.length > 0, `text="${textSeen.slice(0, 60)}"`);
    } catch (e) { check('OpenAI chat e2e', false, e instanceof Error ? e.message : String(e)); }
    try {
    ...
    } catch (e) { check('OpenAI embed e2e', false, e instanceof Error ? e.message : String(e)); }
  }
  ...
  if (!env.NOTION_TOKEN) skipMsg('notion e2e', 'no NOTION_TOKEN');
    ...
    try {
      ...
      const j = await r.json() as { object?: string; type?: string; bot?: { workspace_name?: string } };
    ...
    } catch (e) { check('Notion live', false, e instanceof Error ? e.message : String(e)); }
  }

  // ─── Twilio live ────────────────────────────────────────────────────
  console.log('\n=== Twilio — live /Accounts/{SID}.json ===');
  if (!env.TWILIO_ACCOUNT_SID || !env.TWILIO_AUTH_TOKEN) skipMsg('twilio e2e', 'no TWILIO_*');
  else {
    try {
    ...
    } catch (e) { check('Twilio live', false, e instanceof Error ? e.message : String(e)); }
  }

  // ─── Google Gemini live ─────────────────────────────────────────────
  console.log('\n=== Google Gemini — live /v1beta/models ===');
  if (!env.GOOGLE_GEMINI_API_KEY) skipMsg('gemini e2e', 'no GOOGLE_GEMINI_API_KEY');
  else {
    try {
    ...
    } catch (e) { check('Gemini live', false, e instanceof Error ? e.message : String(e)); }
  }
  ...
  if (!ping.ok) skipMsg('lmstudio live', `not running locally (${ping.error})`);
    ...
    if (lm.models.length > 0) {
      ...
      } catch (e) { check('LM Studio chat e2e', false, e instanceof Error ? e.message : String(e)); }
    }
  }
  ...
  try {
    ...
    if (r.status !== 200) skipMsg('ollama live', `endpoint not reachable (status ${r.status})`);
    else {
      const j = await r.json() as { models: Array<{ name: string }> };
      check('Ollama /api/tags reachable', true);
      check('Ollama lists models', j.models.length > 0, `n=${j.models.length}`);
    }
  } catch (e) { skipMsg('ollama live', `not running (${e instanceof Error ? e.message : String(e)})`); }
