...
describe("ProvenanceService + LanceProvenanceStore", () => {
  ...
  async function svc(
    audit: Parameters<typeof ProvenanceService.prototype.constructor>[3] = null,
  ) {
    h = await tmpLance();
    const store = new LanceProvenanceStore(await h.table(TABLES.provenance));
    return {
      store,
      prov: new ProvenanceService(store, crypto, key, audit ?? null),
    };
  }
  ...
  it("links lineage via parentFp (chunk ← document)", async () => {
    const { prov } = await svc();
    const doc = await prov.record(
      "harvest",
      "doc:report.pdf",
      "document",
      "full document text",
    );
    const chunk = await prov.record(
      "embed",
      "chunk:report.pdf#0",
      "chunk",
      "chunk text",
  ...
  it("verify() fails when a stored signature is tampered", async () => {
    ...
    const prov = new ProvenanceService(
      new LanceProvenanceStore(table),
      crypto,
      key,
      null,
    );
  ...
  it("mirrors high-level ops to the AuditLog sink when provided", async () => {
    ...
    const auditSink = {
      append: async (row: { op: string; afterHash: string | null }) => {
        calls.push(row);
      },
    };
