...
type HttpRecord = { url: string; method: string; headers: Record<string, string>; body?: string };
function makeHttp(routes: Record<string, (req: HttpRecord) => { status: number; body: string }>) {
...
async function main(): Promise<void> {
  ...
  check('Ollama chat yields a text event', chatEvents.some((e) => (e as { type: string }).type === 'text'));
  check('Ollama chat yields a usage event', chatEvents.some((e) => (e as { type: string }).type === 'usage'));
  ...
  type FakeEl = { tagName: string; children: FakeEl[]; textContent: string; value: string; checked: boolean; type: string;
    dataset: Record<string, string>; className: string; appendChild: (c: FakeEl) => FakeEl; setAttribute: (k: string, v: string) => void; addEventListener: (e: string, fn: () => void) => void; empty?: () => void };
  const make = (tag: string): FakeEl => ({
    tagName: tag, children: [], textContent: '', value: '', checked: false, type: '',
    dataset: {}, className: '',
    ...
    addEventListener() { /* not exercised */ },
  ...
  const settingsHost = {
    ...
    setConfig: async <T>(k: string, v: T) => { settingsBlob[k] = v; },
  };
