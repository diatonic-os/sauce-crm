...
import { BudgetGuard, ANTHROPIC_PRICES, OPENAI_PRICES } from "./budgetGuard";
  ...
  try {
    for await (const ev of provider.complete(
      ...
      if (ev.type === "text") d.text += ev.delta;
        ...
        d.toolUses.push({ name: ev.name, input: ev.input });
      else if (ev.type === "usage")
        d.usage = { in: ev.inputTokens, out: ev.outputTokens };
  ...
  const cases: EvalCase[] = [
    {
      ...
      run: async () => {
        ...
        return {
          ok:
            typeof c.streaming === "boolean" && typeof c.toolUse === "boolean",
    ...
    },
    {
      ...
      run: async () => {
        ...
        return {
          ok,
          detail: `text="${d.text.slice(0, 40)}" reason=${d.reason} usage=${d.usage.in}/${d.usage.out}${d.error ? " err=" + d.error : ""}`,
        };
      },
    },
    {
      ...
      run: async () => {
        ...
        let ok = false;
        try {
          for await (const ev of t.provider.complete(
            base({
              stream: true,
              messages: [
        ...
        return {
          ok,
          detail: `deltas=${deltas} reason=${reason} len=${text.length}${err ? " err=" + err : ""}`,
        };
      },
    },
    {
      ...
      run: async () => {
        const d = await drain(
          ...
          base({
            ...
            messages: [
              {
                role: "user",
                content: "Use the get_time tool to tell the time in Tokyo.",
              },
            ],
            tools: [
              {
        ...
        const ok =
          d.reason !== "error" && (d.toolUses.length > 0 || d.text.length > 0);
        return {
          ok,
          detail: `toolUses=${d.toolUses.map((x) => x.name).join(",") || "none"} reason=${d.reason}`,
        };
      },
    },
    {
      ...
      run: async () => {
        ...
        try {
          const d = await drain(
            t.provider,
            base({ model: "this-model-does-not-exist-xyz", stream: false }),
            t.guard,
            t.chatModel,
          );
          ...
          return {
            ok: true,
            detail: errored
              ? `rejected: ${(d.error || "reason=error").slice(0, 50)}`
              : `served leniently (reason=${d.reason})`,
          };
        } catch (e) {
          return {
            ok: true,
            detail: `threw (handled): ${String((e as Error).message).slice(0, 50)}`,
          };
        }
      },
    },
    ...
    cases.push({
      ...
      run: async () => {
        ...
        let ok = false,
          detail = "";
        try {
          const v = await t.provider.embed(
            "the quick brown fox",
            t.embedModel!,
          );
...
async function runProvider(t: ProviderTarget): Promise<void> {
  ...
  for (const c of buildCases(t)) {
    try {
      ...
      console.log(
        `  ⛔ ${c.name} — STOPPED (${stop}): ${String((e as Error).message)}`,
      );
    ...
    if (r.ok) {
      targets.push({
        ...
        guard: new BudgetGuard({
          ...
          maxCalls: 40,
          prices: {},
          fallbackPrice: { inUsdPerMtok: 0, outUsdPerMtok: 0 },
          killFile: KILL_FILE,
        }),
      });
    }
