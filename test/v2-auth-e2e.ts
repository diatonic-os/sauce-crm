...
const nodeCrypto: CryptoBackend = {
  async argon2id(password, salt, opts) {
    return new Promise((resolve, reject) => {
      crypto.scrypt(password, Buffer.from(salt), opts.outBytes, (err, key) =>
...
function mockOAuthHost(opts: {
  tokenResp: () => Record<string, unknown>;
}): OAuthHost & {
  ...
  const state = {
    ...
    lastTokenBody: null as string | null,
    pendingState: "",
  };
  return {
    ...
    async listenOnce(port, path) {
      ...
      cb.searchParams.set(
        "code",
        `mockcode_${Math.random().toString(36).slice(2, 10)}`,
      );
...
async function main(): Promise<void> {
  ...
  const goauth = new OAuthFlow(ghost, vault, subtle as SubtleCrypto, (n) =>
    nodeCrypto.randomBytes(n),
  );
  ...
  const gts: TokenSet = await goauth.authorize("google_workspace", [
    "https://www.googleapis.com/auth/calendar.readonly",
    "https://www.googleapis.com/auth/gmail.readonly",
  ]);
  check(
    "Google authorize URL contains PKCE challenge",
    /code_challenge=/.test(ghost.lastAuthorizeUrl ?? ""),
  );
  check(
    "Google authorize URL uses S256",
    /code_challenge_method=S256/.test(ghost.lastAuthorizeUrl ?? ""),
  );
  check(
    "Google token exchange sent code_verifier",
    /code_verifier=/.test(ghost.lastTokenBody ?? ""),
  );
  check(
    "Google access token captured",
    gts.accessToken === "ya29.mock-google-access",
  );
  check(
    "Google refresh token stored in vault",
    (await vault.get("oauth:google_workspace:refresh")) ===
      "mock-google-refresh",
  );
  check(
    "Google scopes granted",
    goauth.scopesGranted("google_workspace").length === 2,
  );

  // Refresh flow
  const ghost2 = mockOAuthHost({
    tokenResp: () => ({
      access_token: "ya29.mock-google-refreshed",
      refresh_token: "mock-google-refresh-v2",
      expires_in: 3599,
    }),
  });
  const goauth2 = new OAuthFlow(ghost2, vault, subtle as SubtleCrypto, (n) =>
    nodeCrypto.randomBytes(n),
  );
  goauth2.registerProvider("google_workspace", {
    authorizeUrl: "https://accounts.google.com/o/oauth2/v2/auth",
    tokenUrl: "https://oauth2.googleapis.com/token",
    clientId: "c",
    defaultScopes: [],
  ...
  check(
    "Google refresh produces new access token",
    refreshed.accessToken === "ya29.mock-google-refreshed",
  );
  check(
    "Google rotated refresh persisted",
    (await vault.get("oauth:google_workspace:refresh")) ===
      "mock-google-refresh-v2",
  );
  ...
  const fakeProxy = new ProxyClient(
    {
      fetch: async () => ({
        ...
        body: JSON.stringify({ items: [{ id: "evt-1", summary: "Standup" }] }),
      }),
      hmacHex: async () => "sig",
  ...
  const gi = new GoogleWorkspaceIntegration({
    oauth: goauth2,
    scopes,
    proxy: fakeProxy,
  });
  ...
  pulled = r1.pulled;
  check(
    "Google integration syncResource completes",
    typeof r1.pulled === "number",
    `pulled=${pulled}`,
  );
  ...
  console.log("\n=== Microsoft 365 OAuth e2e ===");
  const mhost = mockOAuthHost({
    tokenResp: () => ({
      access_token: "eyJ.mock-ms",
      refresh_token: "mock-ms-refresh",
      expires_in: 3599,
      scope: "Calendars.Read Mail.Read User.Read",
    }),
  });
  const moauth = new OAuthFlow(mhost, vault, subtle as SubtleCrypto, (n) =>
    nodeCrypto.randomBytes(n),
  );
  ...
  const mts = await moauth.authorize("microsoft_365", [
    "Calendars.Read",
    "Mail.Read",
    "User.Read",
  ]);
  ...
  check(
    "Microsoft refresh persisted",
    (await vault.get("oauth:microsoft_365:refresh")) === "mock-ms-refresh",
  );
  check(
    "Microsoft scopes granted include Mail.Read",
    moauth.scopesGranted("microsoft_365").includes("Mail.Read"),
  );

  const mi = new Microsoft365Integration({
    oauth: moauth,
    scopes,
    proxy: fakeProxy,
  });
  ...
  // ─── Notion OAuth E2E ───────────────────────────────────────────────
  console.log("\n=== Notion OAuth e2e ===");
  const nhost = mockOAuthHost({
    tokenResp: () => ({
      access_token: "secret_mock-notion",
      refresh_token: "mock-notion-refresh",
      expires_in: 3600,
    }),
  });
  const noauth = new OAuthFlow(nhost, vault, subtle as SubtleCrypto, (n) =>
    nodeCrypto.randomBytes(n),
  );
  ...
  check(
    "Notion access token captured",
    nts.accessToken === "secret_mock-notion",
  );
  check(
    "Notion token exchange used client_secret",
    /client_secret=/.test(nhost.lastTokenBody ?? ""),
  );
  check(
    "Notion refresh persisted",
    (await vault.get("oauth:notion:refresh")) === "mock-notion-refresh",
  );
  ...
  check(
    "Apple appleid stored",
    (await vault.get("apple:appleid")) === "user@icloud.com",
  );
  check(
    "Apple app-password stored",
    (await vault.get("apple:app-password")) === "abcd-efgh-ijkl-mnop",
  );
  const ai = new AppleIntegration({ scopes, proxy: fakeProxy });
  await ai.connect();
  check(
    "Apple integration connected via stored creds",
    (await ai.state()).connected,
  );
  ...
  check(
    "Twilio SID stored",
    (await vault.get("twilio:account-sid")).startsWith("AC"),
  );
  ...
  check(
    "SMTP host stored",
    (await vault.get("smtp_imap:default:host")) === "imap.fastmail.com",
  );
  check(
    "SMTP password stored",
    !!(await vault.get("smtp_imap:default:password")),
  );
  ...
  const brave = new BraveSearch(
    ...
    async () => await vault.get("web_search:brave"),
  );
  const results = await brave.search("test", { count: 5 });
  check("Brave called with API key from vault", braveCalled);
  check(
    "Brave returns at least one result",
    results.length > 0 && results[0].url === "https://example.com",
  );
  ...
  const realProxy = new ProxyClient(
    ...
    {
      enabled: true,
      baseUrl: "https://proxy.sauce.test",
      sharedSecret: "shared-secret-32-bytes-long-xxxxxx",
    },
  );
  const pr = await realProxy.fetch("https://api.upstream.test/v1/me", {
    method: "POST",
    body: '{"q":1}',
  });
  check(
    "Proxy received correct target",
    proxyTargetSeen === "https://api.upstream.test/v1/me",
  );
