...
async function main(): Promise<void> {
  ...
  await integ.connect();
  check(
    "Integration accepts account + connects",
    (await integ.state()).connected,
  );
  ...
  if (env.IMAP_APP_PASSWORD) {
    await kv.put(
      "smtp_imap:drew_saucetech:app-password",
      env.IMAP_APP_PASSWORD,
    );
  }
  ...
  if (!cred) {
    skipMsg(
      "IMAP probe",
      "no app password in vault or env. Set IMAP_APP_PASSWORD in env.tpl or via Settings.",
    );
  } else {
    console.log("\n--- Live IMAP TLS handshake + LOGIN exchange ---");
    const result = await integ.probeAccount("drew_saucetech");
    check(
      "TLS handshake completes against imap.gmail.com:993",
      result.capability.length > 0 || !!result.greeting,
      `greeting="${result.greeting.slice(0, 50)}"`,
    );
    check(
      "Server greeting parsed",
      /\* OK/.test(result.greeting),
      `g="${result.greeting.slice(0, 40)}"`,
    );
    check(
      "CAPABILITY enumerated",
      result.capability.length > 0,
      `caps=[${result.capability.slice(0, 4).join(", ")}…]`,
    );
    if (result.ok) {
      check(
        "AUTH succeeded end-to-end (live!)",
        result.ok,
        `latency=${result.loginLatencyMs}ms, inbox=${result.messageCount} msgs`,
      );
    } else {
      // Common Gmail rejection codes — these prove the protocol layer worked, only the cred is bad
      const expected =
        /AUTHENTICATIONFAILED|app password|application-specific|invalid credentials/i.test(
          result.error ?? "",
        );
      check(
        "AUTH failure cleanly surfaced (cred state, not code path)",
        expected,
  ...
  if (!xtok)
    skipMsg(
      "XOAUTH2 live probe",
      "no OAuth access token (set via Google Workspace OAuth integration first)",
    );
  else {
    ...
    if (xresult.ok)
      ...
      check(
        "XOAUTH2 AUTH failure surfaced cleanly",
        !!xresult.error,
        `error="${xresult.error?.slice(0, 80)}"`,
      );
  }
  ...
  try {
    ...
    if ((e as Error).message?.includes("Scope not granted")) threw = true;
  }
  ...
  check(
    "SOCKS5 misroute fails closed (not silent plaintext fallback)",
    !proxyResult.ok,
    `error="${proxyResult.error?.slice(0, 60)}"`,
  );
  ...
  check(
    ...
    strictResult.capability.length > 0 ||
      /AUTHENTICATION|app password/i.test(strictResult.error ?? ""),
